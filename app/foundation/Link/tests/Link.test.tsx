  ...
  describe('url', () => {
    describe('internal', () => {
      it('renders a react router link', () => {
        ...
        expect(link.find(ReactRouterLink)).toHaveLength(1);
      });
      ...
      it('does not set external attributes', () => {
        const link = mount(<Link url="/home" />);
        const props = link.find(ReactRouterLink).props();
    ...
    describe('external', () => {
      ...
      it('sets default external attributes for internal links explicitly set to external', () => {
        const link = mount(<Link url="https://shopify.com" external />);
        expect(link.find(ReactRouterLink).props()).toMatchObject({
          target: '_blank',
          rel: 'noopener noreferrer',
